# app.py - SYNTAX CHECK PASS 6 - Meticulous Python Review
# --- Imports ---
...
# --- Helper Function: get_edge_color ---
...
def get_luminance(r, g, b):
    ...
    try:
        ...
        # Standard coefficients for relative luminance
...
# --- Helper Function: get_books_from_shelf (Simplified Author Logic) ---
                ...
                if title_elem and author_elem:
                    ...
                    if ', ' in author_text:
                        ...
                        if len(parts) == 2:
                            ...
                            author_name = author_text # Fallback
                    else:
                    ...
                    if page_count_elem:
                        ...
                        if match:
                            try:
                                ...
                                if parsed_num > 0: page_count = parsed_num
                            except ValueError:
                                pass
...
print("Test change - Version 1.0")

# --- HTML Template (JS reverted to state before DOMContentLoaded change) ---
...
<head>
    ...
    <style>
        ...
        #detail-right #detail-publisher, 
        #detail-right #detail-page-count { 
            font-size: 1em; 
...
<body>
    ...
    <div id="detail-view"> <div id="detail-left"></div> <div id="detail-right"> <h2 id="detail-title">Book Title</h2> <p id="detail-author">Author Name</p> <p id="detail-publisher">Publisher</p> <p id="detail-page-count">XXX pages</p> <hr> <p><strong>My Rating:</strong> <em style="color:#aaa;">(Not Available)</em></p> <p><strong>My Review:</strong> <em style="color:#aaa;">(Not Available)</em></p> <p><strong>Description:</strong> <em style="color:#aaa;">(Not Available)</em></p> </div> <button id="close-detail-button" title="Close Details">&times;</button> </div>
    ...
    <script type="module">{% raw %}
        ...
        function onWindowScroll() {
            ...
            const actualTotalStackHeight = booksGroup.children.reduce((sum, mesh) => 
                sum + mesh.geometry.parameters.height + BOOK_SPACING, 0) - BOOK_SPACING;
            
            // Ensure we have a valid scroll range
            const scrollableHeight = document.documentElement.scrollHeight - window.innerHeight;
            if (scrollableHeight <= 0) return;

            currentScrollY = window.scrollY;
            const scrollRatio = Math.max(0, Math.min(1, currentScrollY / scrollableHeight));
            
            // Calculate target position with bounds checking
            const startY = actualTotalStackHeight / 2;
            const initialGroupY = -startY + (booksGroup.children[0]?.geometry.parameters.height / 2 || 0);
            const maxTravel = actualTotalStackHeight - (booksGroup.children[0]?.geometry.parameters.height || BOOK_DEFAULTS.HEIGHT);
            
            targetGroupY = initialGroupY + (scrollRatio * maxTravel);
        }
        ...
        let isScrolling = false; let scrollTimeout; function onDetailScroll(event) { /* SCROLL BETWEEN DETAILS DISABLED */ if (true) { if (isDetailView) event.preventDefault(); return; } }
        ...
        function showDetailView(index) {
            ...
            if (!meshToShow || !bookData[index]) {
                console.error("Invalid book mesh or data");
                isTransitioning = false;
                return;
            }

            // Store original state for return animation
            meshToShow.userData.originalState = {
                position: meshToShow.position.clone(),
                rotation: meshToShow.rotation.clone(),
                scale: meshToShow.scale.clone()
            };
            ...
            // Reset detail view scroll
            detailViewDiv.scrollTop = 0;

            // Create animation timeline
            const tl = gsap.timeline({
                onStart: () => {
                    console.log("Detail animation starting");
                    detailViewDiv.style.pointerEvents = 'auto';
                    detailViewDiv.classList.add('visible');
                    
                    // Ensure selected book is visible and on top
                    meshToShow.visible = true;
                    meshToShow.renderOrder = 1;
                },
                onComplete: () => {
                    console.log("Detail animation complete");
                    isDetailView = true;
                    selectedBookIndex = index;
                    selectedBookMesh = meshToShow;
                    isTransitioning = false;
                }
            });

            // Animate other books away
            booksGroup.children.forEach((mesh, i) => {
                if (i !== index) {
                    tl.to(mesh, {
                        x: (i % 2 === 0 ? -1 : 1) * DETAIL_VIEW.OTHER_BOOKS.EXIT_X,
                        opacity: 0,
                        visible: false,
                        duration: DETAIL_VIEW.OTHER_BOOKS.FADE_DURATION,
                        ease: "power2.inOut"
                    }, 0);
                }
            });

            // Animate selected book to detail position
            tl.to(meshToShow.position, {
                x: DETAIL_VIEW.BOOK.POSITION.x,
                y: DETAIL_VIEW.BOOK.POSITION.y,
                z: DETAIL_VIEW.BOOK.POSITION.z,
                duration: DETAIL_VIEW.BOOK.ANIMATION_DURATION,
                ease: DETAIL_VIEW.BOOK.EASE
            }, 0);

            tl.to(meshToShow.rotation, {
                x: DETAIL_VIEW.BOOK.ROTATION.x,
                y: DETAIL_VIEW.BOOK.ROTATION.y,
                z: DETAIL_VIEW.BOOK.ROTATION.z,
                duration: DETAIL_VIEW.BOOK.ANIMATION_DURATION,
                ease: DETAIL_VIEW.BOOK.EASE
            }, 0);

            tl.to(meshToShow.scale, {
                x: DETAIL_VIEW.BOOK.SCALE,
                y: DETAIL_VIEW.BOOK.SCALE,
                z: DETAIL_VIEW.BOOK.SCALE,
                duration: DETAIL_VIEW.BOOK.ANIMATION_DURATION,
                ease: DETAIL_VIEW.BOOK.EASE
            }, 0);
        ...
        function hideDetailView() {
            ...
            const meshToRestore = selectedBookMesh;
            const originalState = meshToRestore.userData.originalState;

            if (!originalState) {
                console.error("Missing original state for book restoration");
                isTransitioning = false;
                return;
            }
            ...
            const tl = gsap.timeline({
                onStart: () => {
                    ...
                    detailViewDiv.style.pointerEvents = 'none';
                    
                    // Make all books visible again
                ...
                onComplete: () => {
                    ...
                    isDetailView = false;
                    selectedBookIndex = -1;
                    selectedBookMesh = null;
                    isTransitioning = false;
                    detailViewDiv.classList.remove('visible');
                    
                    // Reset all books to their original positions
                    booksGroup.children.forEach(mesh => {
                        if (mesh.userData.stackY !== undefined) {
                            mesh.position.set(0, mesh.userData.stackY, 0);
                            mesh.rotation.set(TARGET_ROTATION_X, TARGET_ROTATION_Y, 0);
            ...
            // Animate selected book back to shelf
            tl.to(meshToRestore.position, {
            ...
            // Animate other books back in
            booksGroup.children.forEach((mesh, i) => {
                if (mesh !== meshToRestore) {
                    tl.to(mesh, {
                        x: 0,
                        opacity: 1,
                        visible: true,
                        duration: DETAIL_VIEW.OTHER_BOOKS.FADE_DURATION,
                        ease: "power2.inOut"
                    }, DETAIL_VIEW.BOOK.ANIMATION_DURATION * 0.5);
                }
            });
        }
        function animate() {
            ...
            // Smooth scroll animation only when not in detail view
            if (!isDetailView && !isTransitioning) {
                const diff = targetGroupY - booksGroup.position.y;
                if (Math.abs(diff) > 0.01) {
                    // Smoother interpolation
                    booksGroup.position.y += diff * 0.15;
                }
            }
        ...
        function setupEventHandlers() {
            shelfForm.addEventListener('submit', handleUrlSubmit);
            window.addEventListener('resize', onWindowResize);
            window.addEventListener('mousemove', onMouseMove);
            window.addEventListener('click', onClick);
            window.addEventListener('wheel', onDetailScroll, { passive: false });
            closeDetailButton.addEventListener('click', hideDetailView);
        }
...
# --- Flask Routes (Correctly Formatted) ---
# ... (index, get_books_api, get_progress - verified correct format) ...
@app.route("/")
def index():
...
def get_books_api():
    ...
    elif error_message: return jsonify({"error": f"Warning: {error_message}", "books": books_data or [], "total_found": len(books_data or [])}), 200
